...
uv run pytest --cov=qmd_py --cov-report=term-missing  # then: coverage report --format=markdown
...
(README's "tested" links to it). It goes stale silently — refresh it with
the command above when the numbers actually move. Only the Markdown
summary is tracked: the HTML report is a build artifact, stays gitignored
as `htmlcov/`, and is CI's job to publish once the project moves to
